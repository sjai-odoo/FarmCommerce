...
class ContactProperty(models.Model):
    ...
    purchase_offer_ids = fields.One2many('purchase.order.line','purchase_id')
    buyer_id = fields.Many2one('res.partner', string='Buyer', copy=False)
    status = fields.Selection(string='Status', selection=[('place','Place Order'), ('draft','Draft'), ('ordered','Ordered'), ('delivered','Delivered')], default='place', copy=False)
